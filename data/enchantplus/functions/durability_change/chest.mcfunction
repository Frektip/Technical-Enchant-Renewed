###Debug Message###
tellraw @a[tag=teplus.pydbgm] ["",{"text":"TE+» ","color":"#65DAD4","bold":true},{"text":"Changing the durability from the player's ","color":"gray","italic":true,"bold":false},{"text":"chest","color":"dark_aqua","italic":true,"bold":false}]
...
data modify storage teplus:item_dura item.Damage[0] set from entity @s[type=player] Inventory[{Slot:102b}].tag.Damage
data modify storage teplus:item_dura item.Damage[0] set from entity @s[type=!player] ArmorItems[2].tag.Damage
#Damage the item completely and get it's max value in the right
item modify entity @s armor.chest enchantplus:lose_dura
data modify storage teplus:item_dura item.Damage[1] set from entity @s[type=player] Inventory[{Slot:102b}].tag.Damage
data modify storage teplus:item_dura item.Damage[1] set from entity @s[type=!player] ArmorItems[2].tag.Damage
