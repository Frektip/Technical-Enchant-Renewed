###Debug Message###
tellraw @a[tag=teplus.pydbgm] ["",{"text":"TE+» ","color":"#65DAD4","bold":true},{"text":"Changing the durability from the player's ","color":"gray","italic":true,"bold":false},{"text":"offhand","color":"dark_aqua","italic":true,"bold":false}]
...
data modify storage teplus:item_dura item.Damage[0] set from entity @s[type=player] Inventory[{Slot:-106b}].tag.Damage
data modify storage teplus:item_dura item.Damage[0] set from entity @s[type=!player] HandItems[1].tag.Damage
#Damage the item completely and get it's max value in the right
item modify entity @s weapon.offhand enchantplus:lose_dura
data modify storage teplus:item_dura item.Damage[1] set from entity @s[type=player] Inventory[{Slot:-106b}].tag.Damage
data modify storage teplus:item_dura item.Damage[1] set from entity @s[type=!player] HandItems[1].tag.Damage
...
execute unless score .maxdmg tep.rc matches 0 if score .tempdmg tep.rc < .maxdmg tep.rc run item modify entity @s weapon.offhand enchantplus:copy_damage

#If the Damage score exceeds the max score, clear the item
# Do it for certain custom enchantments
execute unless score .maxdmg tep.rc matches 0 if score .maxdmg tep.rc matches 1.. if score .tempdmg tep.rc >= .maxdmg tep.rc run item replace entity @s weapon.offhand with air
...
execute unless score .maxdmg tep.rc matches 0 if score .maxdmg tep.rc matches 1.. if score .tempdmg tep.rc >= .maxdmg tep.rc if entity @s[nbt={Inventory:[{Slot:-106b,tag:{CustomEnchantments:[{id:"Splatter"}]}}]}] run function enchantplus:result/splatter/clear_rod
