...
data modify storage teplus:item_dura item set from entity @s Inventory[{Slot:-106b}]
...
execute if score .tempdmg tep.rc >= .maxdmg tep.rc run item replace entity @s weapon.offhand with air
execute if score .tempdmg tep.rc >= .maxdmg tep.rc run playsound entity.item.break master @a ~ ~ ~ 2 1
execute if score .tempdmg tep.rc >= .maxdmg tep.rc if entity @s[nbt={Inventory:[{Slot:-106b,tag:{CustomEnchantments:[{id:"Splatter"}]}}]}] run function enchantplus:result/splatter/clear_rod


#If the Damage score is less than the max score, copy the new nbt from
# the storage to the player
execute if score .tempdmg tep.rc < .maxdmg tep.rc run item modify entity @s weapon.offhand enchantplus:copy_nbt