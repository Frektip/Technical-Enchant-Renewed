...
data modify storage teplus:item_lore First set value [{id:"XpBoost",lvl:1}]

#Set a random level value from the max level limit scoreboard
#Prepare the random value
scoreboard players set $min random 1
execute store result score $max random run scoreboard players get #xpbt teplus.maxlvl
...
loot spawn ~ ~ ~ loot enchantplus:enchanted_book/xp_boost
