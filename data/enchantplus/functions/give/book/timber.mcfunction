...
data modify storage teplus:item_lore First set value [{id:"Timber",lvl:1}]

#Set a random level value from the max level limit scoreboard
#Prepare the random value
scoreboard players set $min random 1
execute store result score $max random run scoreboard players get #tmbr teplus.maxlvl
...
loot spawn ~ ~ ~ loot enchantplus:enchanted_book/timber
