...
execute unless predicate enchantplus:held_item/have_boots run tellraw @s [{"text":"\n\n\n"},{"translate":"Error! ","color":"dark_red","bold":true},{"translate":"You can't add this enchantment to this item","color":"red","bold":false},{"text":"\n\n\n"}]
#-------------------------------------------------------------------------------#

##Store the original value in the storage
execute if predicate enchantplus:held_item/have_boots run data modify storage teplus:heldench Mainhand set from entity @s SelectedItem

##Summon an armor stand and tag the player if is holding the right item
execute if predicate enchantplus:held_item/have_boots run tag @s add teplus.heldench
execute if predicate enchantplus:held_item/have_boots run summon armor_stand ~ ~ ~ {Marker:1b,Tags:["teplus.held_ench"],HandItems:[{},{}],DisabledSlots:4144959}
#-------------------------------------------------------------------------------#


##FIRST check if the item already has this enchantment
execute if data storage teplus:heldench Mainhand.tag.CustomEnchantments[{id:"Agility",lvl:1}] run tag @s add teplus.heldench_error
...
execute if entity @s[tag=teplus.heldench] run data remove storage teplus:heldench Mainhand.tag.CustomEnchantments[{id:"Agility"}]

#3.- Check if the item doesn't have this enchantment previously
execute if entity @s[tag=teplus.heldench] unless data storage teplus:heldench Mainhand.tag.CustomEnchantments[{id:"Agility",lvl:1}] run data modify storage teplus:heldench Mainhand.tag.CustomEnchantments append value {id:"Agility",lvl:1}
...
execute if entity @s[tag=teplus.heldench] if data storage teplus:heldench Mainhand.tag.CustomEnchantments[{id:"Agility",lvl:1}] as @e[type=armor_stand,tag=teplus.held_ench,distance=..1,limit=1] run item modify entity @s weapon.mainhand enchantplus:set_attributes/heldenchant/agility
