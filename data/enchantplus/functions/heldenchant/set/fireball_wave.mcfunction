...
execute unless entity @s[nbt={SelectedItem:{id:"minecraft:crossbow"}}] run tellraw @s [{"text":"\n\n\n"},{"translate":"Error! ","color":"dark_red","bold":true},{"translate":"You can't add this enchantment to this item","color":"red","bold":false},{"text":"\n\n\n"}]
#-------------------------------------------------------------------------------#

##Store the original value in the storage
execute if entity @s[nbt={SelectedItem:{id:"minecraft:crossbow"}}] run data modify storage teplus:heldench Mainhand set from entity @s SelectedItem

##Summon an armor stand and tag the player if is holding the right item
execute if entity @s[nbt={SelectedItem:{id:"minecraft:crossbow"}}] run tag @s add teplus.heldench
execute if entity @s[nbt={SelectedItem:{id:"minecraft:crossbow"}}] run summon armor_stand ~ ~ ~ {Marker:1b,Tags:["teplus.held_ench"],HandItems:[{},{}],DisabledSlots:4144959}
#-------------------------------------------------------------------------------#


##FIRST check if the item already has this enchantment
execute if data storage teplus:heldench Mainhand.tag.CustomEnchantments[{id:"FireballWave",lvl:1}] run tag @s add teplus.heldench_error
...
execute if entity @s[tag=teplus.heldench] unless data storage teplus:heldench Mainhand.tag.CustomEnchantments[{id:"FireballWave",lvl:1}] run data modify storage teplus:heldench Mainhand.tag.CustomEnchantments append value {id:"FireballWave",lvl:1}
#2.1.- Check for "CustomArrow" nbt tag ONLY for crossbow/crosscrossbow
execute if entity @s[tag=teplus.heldench] unless data storage teplus:heldench Mainhand.tag.CustomArrow run data modify storage teplus:heldench Mainhand.tag.CustomArrow append value 1
