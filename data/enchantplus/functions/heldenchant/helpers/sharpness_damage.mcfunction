...
execute store result storage teplus:shrpatt Amount double 0.001 run data get entity @s HandItems[0].tag.AttributeModifiers[{AttributeName:"minecraft:generic.attack_damage"}].Amount 10000
...
execute store result score #teplus.sharp.lvl teplus.data run data get entity @s HandItems[0].tag.Enchantments[{id:"minecraft:sharpness"}].lvl
#Scale it by 10
...
#Set the new damage value in the HandItems[0]
execute store result entity @s HandItems[0].tag.AttributeModifiers[{AttributeName:"minecraft:generic.attack_damage"}].Amount double 0.1 run scoreboard players get #teplus.attib.out teplus.data
