...
execute store result score #held.lvl teplus.data run data get storage teplus:heldench Mainhand.tag.CustomEnchantments[{id:"BigPath"}].lvl

#Compare it to the main scoreboard Config
# First check if it is equal or greater than the current max level limit
execute if score #held.lvl teplus.data >= #bgpt teplus.maxlvl run tag @s add teplus.heldench_error

#If not, then keep adding one level to it
execute if score #held.lvl teplus.data < #bgpt teplus.maxlvl run scoreboard players add #held.lvl teplus.data 1
...
execute if entity @s[tag=teplus.heldench] unless data storage teplus:heldench Mainhand.tag.CustomEnchantments[{id:"BigPath"}] run data modify storage teplus:heldench Mainhand.tag.CustomEnchantments append value {id:"BigPath",lvl:1}

#2.1.- Add the respective level to it
execute if entity @s[tag=teplus.heldench] store result storage teplus:heldench Mainhand.tag.CustomEnchantments[{id:"BigPath"}].lvl int 1 run scoreboard players get #held.lvl teplus.data
