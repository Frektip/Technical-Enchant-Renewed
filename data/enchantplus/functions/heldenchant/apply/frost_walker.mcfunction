...
execute store result score #held.lvl teplus.data run data get storage teplus:heldench Mainhand.tag.Enchantments[{id:"minecraft:frost_walker"}].lvl

#Compare it to the main scoreboard Config
# First check if it is equal or greater than the current max level limit
execute if score #held.lvl teplus.data matches 2.. run tag @s add teplus.heldench_error

#If not, then keep adding one level to it
execute if score #held.lvl teplus.data matches ..1 run scoreboard players add #held.lvl teplus.data 1
...
#1.- Check if the item doesn't have this enchantment previously
execute if entity @s[tag=teplus.heldench] unless data storage teplus:heldench Mainhand.tag.Enchantments[{id:"minecraft:frost_walker"}] run data modify storage teplus:heldench Mainhand.tag.Enchantments append value {id:"minecraft:frost_walker",lvl:1}

#2.- Add the respective level to it
execute if entity @s[tag=teplus.heldench] store result storage teplus:heldench Mainhand.tag.Enchantments[{id:"minecraft:frost_walker"}].lvl int 1 run scoreboard players get #held.lvl teplus.data
...
#4.- Give to the player the final result of the item
