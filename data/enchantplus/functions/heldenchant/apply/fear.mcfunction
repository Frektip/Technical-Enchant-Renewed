...
execute store result score #held.lvl teplus.data run data get storage teplus:heldench Mainhand.tag.CustomCurse[{id:"Fear"}].lvl

#Compare it to the main scoreboard Config
# First check if it is equal or greater than the current max level limit
execute if score #held.lvl teplus.data >= #fear teplus.maxlvl run tag @s add teplus.heldench_error

#If not, then keep adding one level to it
execute if score #held.lvl teplus.data < #fear teplus.maxlvl run scoreboard players add #held.lvl teplus.data 1
...
execute if entity @s[tag=teplus.heldench] unless data storage teplus:heldench Mainhand.tag.CustomCurse[{id:"Fear"}] run data modify storage teplus:heldench Mainhand.tag.CustomCurse append value {id:"Fear",lvl:1}

#2.1.- Add the respective level to it
execute if entity @s[tag=teplus.heldench] store result storage teplus:heldench Mainhand.tag.CustomCurse[{id:"Fear"}].lvl int 1 run scoreboard players get #held.lvl teplus.data

#2.2.- Check for "CustomArrow" nbt tag ONLY for bow/crossbow
execute if entity @s[tag=teplus.heldench] if predicate enchantplus:held_item/have_bow unless data storage teplus:heldench Mainhand.tag.CustomArrow run data modify storage teplus:heldench Mainhand.tag.CustomArrow set value 1
execute if entity @s[tag=teplus.heldench] if predicate enchantplus:held_item/have_crossbow unless data storage teplus:heldench Mainhand.tag.CustomArrow run data modify storage teplus:heldench Mainhand.tag.CustomArrow set value 1
