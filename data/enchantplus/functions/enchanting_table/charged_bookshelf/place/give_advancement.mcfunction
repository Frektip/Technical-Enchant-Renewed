...
execute as @e[type=marker,tag=teplus.charged_bookshelf_init] at @s if entity @e[type=marker,tag=teplus.tier3,distance=2..3,sort=nearest] run advancement grant @p[advancements={enchantplus:progress/start/charged_bookshelf_challenge=false}] only enchantplus:progress/start/charged_bookshelf_challenge
