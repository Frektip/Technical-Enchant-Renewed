...
execute if score #diench game.Opts matches 0 if entity @s[scores={Drop=1..}] as @e[type=item,distance=..3,nbt={Item:{tag:{CustomEnchantments:[{}]}}},limit=1] at @s if block ~ ~-1 ~ grindstone run function enchantplus:disenchant/core
##-----#-----#-----#-----#-----#-----#-----#-----#-----#-----#-----#-----#-----#-----#-----#
