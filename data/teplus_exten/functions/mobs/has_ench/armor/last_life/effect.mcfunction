...
#effects
execute if entity @s[type=#enchantplus:undead] run effect give @s minecraft:instant_damage 1 1 true
execute unless entity @s[type=#enchantplus:undead] run effect give @s minecraft:instant_health 1 1 true
particle minecraft:damage_indicator ~ ~ ~ 0.5 1 0.5 0.1 10 force
playsound minecraft:item.totem.use ambient @a[distance=..15] ~ ~ ~ 1
