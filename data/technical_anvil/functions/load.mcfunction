scoreboard objectives add tep.avl dummy

scoreboard objectives add TCHA.Value dummy
scoreboard objectives add TCHA.plxp dummy
scoreboard objectives add TCHA.book dummy

scoreboard objectives add attb.armor dummy
scoreboard objectives add attb.knbrs dummy
scoreboard objectives add attb.toug dummy
scoreboard objectives add attb.dmg dummy

scoreboard objectives add TCHA.Section1 dummy
scoreboard objectives add TCHA.Section2 dummy

scoreboard objectives add teplus.vgmrl dummy

scoreboard objectives add teplus.antkbk dummy
scoreboard objectives add teplus.hrds dummy

#Run other important functions
function technical_anvil:prepare_ui
function technical_anvil:main