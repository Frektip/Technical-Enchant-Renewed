...
execute if score @s TCHA.Section1 matches 1 run data modify storage teplus:tech_anvil.ui ResultItem.tag.Enchantments[{id:"minecraft:frost_walker"}].lvl set value 1s
execute if score @s TCHA.Section1 matches 2 run data modify storage teplus:tech_anvil.ui ResultItem.tag.Enchantments[{id:"minecraft:frost_walker"}].lvl set value 2s
tag @s add CEitm