...
# Add 1 to the page scoreboard
scoreboard players add @s teplus.admin_box.page 1

#----- Look for the new group of enchantments -----#
# Remove 19 by 19 according to the pages
# Set it to 19 by default
scoreboard players set %remove teplus.admin_box.data 19
...
# In case it reaches the final page and there are blank spaces add them
execute if score @s teplus.admin_box.page matches 2.. if score @s teplus.admin_box.page >= $page.lim teplus.admin_box.data if score $extra.pg teplus.admin_box.data matches 1.. run function teplus:admin/box/enchants_menu/helper/add_blank_spaces
...
# Add navigation buttons (detect if it is in a middle or last page)
...
execute if score @s teplus.admin_box.page matches 2.. if score @s teplus.admin_box.page >= $page.lim teplus.admin_box.data run data modify block ~ ~ ~ Items append from storage teplus:ui Global[{profile:"admin_box"}].Nav.End[]
