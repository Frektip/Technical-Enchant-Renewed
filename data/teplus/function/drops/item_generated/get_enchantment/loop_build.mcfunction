...
# If it there is an enchantment, then store it
# Reuse the same "success" when enchanting a book from the enchanting table
execute if data storage teplus:loot Build.id run function teplus:enchant/set_enchant/success

# Loop until the Copy Storage is empy
data remove storage teplus:loot Copy[0]
execute if data storage teplus:loot Copy[0].id run function teplus:drops/item_generated/get_enchantment/loop_build