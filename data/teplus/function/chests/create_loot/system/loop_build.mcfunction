...
execute if score #is_enabled teplus.data matches 0 run data remove storage teplus:chest Build

# If it is enabled continue with the main process. Only if the enchantment has the Structure[] nbt tag
execute if score #is_enabled teplus.data matches 1 if data storage teplus:chest Build.Structure run function teplus:chests/create_loot/system/build

# Loop until the Copy Storage is empy
data remove storage teplus:chest Copy[0]
execute if data storage teplus:chest Copy[0].id run function teplus:chests/create_loot/system/loop_build